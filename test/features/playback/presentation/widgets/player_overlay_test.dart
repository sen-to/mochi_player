...
void main() {
  testWidgets('aligns title with the safe inset after macOS window controls', (tester) async {
    ...
    try {
      await tester.pumpWidget(
        _testApp(
          PlayerTopBar(title: '进击的巨人', secondaryTitle: '第 1 季 · 第 1 集', cacheSpeed: '3.2 MB/s', isFullScreen: false),
        ),
      );

      expect(tester.getTopLeft(find.text('进击的巨人')).dx, WindowControlsLayout.leadingContentInset);
  ...
  test('uses the regular page inset on Windows', () {
    expect(PlayerOverlayLayout.topLeftInset(platform: TargetPlatform.windows, isFullScreen: false), AppSpacing.xxl);
  });

  testWidgets('uses the regular page inset in fullscreen', (tester) async {
    await tester.binding.setSurfaceSize(const Size(1200, 700));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    await tester.pumpWidget(_testApp(PlayerTopBar(title: '电影', cacheSpeed: '3.2 MB/s', isFullScreen: true)));
  ...
  testWidgets('hides the cache speed when it is unavailable', (tester) async {
    await tester.pumpWidget(_testApp(PlayerTopBar(title: '电影', cacheSpeed: '', isFullScreen: true)));

    expect(find.byKey(const ValueKey('player-cache-speed')), findsNothing);
  });

  testWidgets('keeps the bottom glass panel inset and progress above controls', (tester) async {
    await tester.binding.setSurfaceSize(const Size(1200, 700));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    await tester.pumpWidget(
      _testApp(
        const Align(
          ...
          child: PlayerBottomControlBar(
            ...
            controls: SizedBox(key: ValueKey('controls'), height: PlayerOverlayLayout.controlHeight),
          ),
        ),
      ),
    );
  ...
  testWidgets('lets the bottom control bar move within the player bounds', (tester) async {
    var buttonPresses = 0;
    Rect? reportedBounds;
    await tester.binding.setSurfaceSize(const Size(1200, 700));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    ...
    expect(find.byKey(const ValueKey('player-control-bar-drag-handle')), findsNothing);
    await tester.dragFrom(
      originalTopLeft + Offset(tester.getSize(panel).width - 8, tester.getSize(panel).height - 8),
      const Offset(80, -100),
    );
  ...
  test('scales subtitle text continuously with the player viewport', () {
    expect(PlayerSubtitleSizing.fontSize(configuredFontSize: 32, viewportSize: const Size(1200, 700)), 32);
    expect(PlayerSubtitleSizing.fontSize(configuredFontSize: 32, viewportSize: const Size(480, 300)), 12.8);
    expect(PlayerSubtitleSizing.fontSize(configuredFontSize: 32, viewportSize: const Size(900, 600)), 24);
  });

  testWidgets('keeps subtitles at the bottom unless the control bar overlaps them', (tester) async {
    await tester.binding.setSurfaceSize(const Size(1200, 700));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    ...
    Widget subtitleLayout(Rect controlBarBounds) => _testApp(
      PlayerSubtitleAvoidingControls(
        ...
        child: const SizedBox(key: ValueKey('test-subtitle'), width: 240, height: 40),
      ),
    );

    await tester.pumpWidget(subtitleLayout(const Rect.fromLTWH(252, 624, 696, 64)));
    expect(tester.getTopLeft(find.byKey(const ValueKey('test-subtitle'))).dy, 580);

    await tester.pumpWidget(subtitleLayout(const Rect.fromLTWH(252, 524, 696, 64)));
    expect(tester.getTopLeft(find.byKey(const ValueKey('test-subtitle'))).dy, 640);
  });

  testWidgets('keeps subtitles at the bottom while controls are hidden', (tester) async {
    await tester.binding.setSurfaceSize(const Size(1200, 700));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    await tester.pumpWidget(
      _testApp(
        const PlayerSubtitleAvoidingControls(
          ...
          child: SizedBox(key: ValueKey('test-subtitle'), width: 240, height: 40),
        ),
      ),
    );
  ...
  testWidgets('mini-player exposes playback, pin and restore controls', (tester) async {
    await tester.pumpWidget(
      _testApp(
        Center(
    ...
    expect(tester.getSize(find.byKey(const ValueKey('player-mini-controls'))), const Size(106, 38));
